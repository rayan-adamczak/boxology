...
export function Logo({ hauteur = 18, className }: { hauteur?: number; className?: string }) {
  /* Une seule dimension pilotée, la hauteur ; la largeur suit le rapport natif,
     99,5604 × 111,8802. Même règle que la visionneuse : c'est ce qui rend la
     déformation impossible.

     Les tranches sont espacées de 3,6 unités de plus chacune (`translate` sur
     l'ambre et le rouge), ce qui porte la gouttière entre fûts de 6,4 à 10 et
     élargit le motif de 7,2. Le `viewBox` suit, sinon le décalage serait rogné
     à droite. Les tracés, eux, ne sont pas retouchés : le décalage est une
     transformation, donc réversible d'un seul nombre. */
  const largeur = (hauteur * 99.5604) / 111.8802;

  return (
    <svg
      viewBox="28.5596 33.7028 99.5604 111.8802"
      width={largeur}
      height={hauteur}
      className={className}
      aria-hidden="true"
